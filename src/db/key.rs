...
pub fn get_secret_keys(
    ...
    let tsk = t_details
        .and_then(|d| d.sk)
        .and_then(|sk| SecretKey::from_str(&sk).ok());
    let zsk = z_details.and_then(|d| d.sk).and_then(|sk| {
        decode_extended_spending_key(network.hrp_sapling_extended_spending_key(), &sk).ok()
    });
    let osk = o_details
        .and_then(|d| d.sk)
        .and_then(|sk| Some(SpendingKey::from_bytes(sk).unwrap()));
